...
# -*- coding:utf-8 -*-
"""
Equations related to simulating blood flow.
Contains a 1-D pulsatile blood flow solver (WIP).
"""
import copy
import math
...
from Blood_Flow_1D import Results


def flux_function(self, area):
    """
    Inlet function to solve
...
def chrt_function(self, area):
    """
    Characteristic function for the outlets.
    Related to the method of caracteristics
    :param self:
    :param area:
    :return:
...
def thickness(r):
    ...
    :return: thickness defined in mm
    """
    radius = r * 1e-3
    h1 = 0.2802
    h2 = -5.053 * 1e2
...
def SolveOutletRCR(args):
    """
    Method to solve for the state variables for the next iteration at the outlets.
...
def SolveInlet(args):
    """
    Method to solve for the state variables for the next iteration at the inlets.
...
class Bifurcation:
    def __init__(self, nodes, signs):
        """
        Initiate the bifurcation with a set of nodes and their positive direction.
        :param nodes: list of nodes connected to this bifurcation
        :param signs: list of signs that define the positive direction of flow at the nodes
        """
        self.Nodes = nodes
        self.Signs = signs
    ...
    def Jac(self, values):
        """
        Method to compute the jacobian of funcs at the bifurcation.
        :param values: List of flow velocity and area (lumen) at the connected nodes.
        :return: Jacobian
        """
    ...
    def SolveBif(self):
        """
        Method to update the state variables for the next iteration.
        Functions to minimize at the bifurcations.
        1) mass conservation
        2) continuity of characteristics
        3) continuity of pressure
...
def SolvingBif(args):
    """
    Method to call the solve bifurcation function in parallel.
...
def SolveVessel(vessel):
    """
    Maccormack method to solve the for the state variables inside a vessel.
...
def FlowRateAlastruey2007function(t, period, scaling):
    """
    Updated function from FlowRateAlastruey2007
    :param t: Current time
    :param period: Period of the function
    :param scaling: Scaling to obtain an integrated volume flow rate equal to this param.
    :return: Flow value
...
def Bloodflow1D(self):
    """
    Function to simulation pulsatile blood flow in the system defined in self.
    ...
    for vessel in self.Topology.Vessels:
        ...
        vessel.CurrentVelocity = [0 for _ in vessel.Nodes]
    ...
    def flowrate(t):
        """
        internal function to calculate the flow rate
        :param t: time values
        :return: flow rates/
        """
    ...
    def Update(vessel):
        """
        Update the vessels such that the current time step becomes the last.
...
def murraylaw(radius, exponent=3):
    """
    Function to calculate the radius of daughter branches based on Murray's law.
    Assume symmetric branches.
    :param radius: Mother radius
    :param exponent: exponent of Murray's law: default:3
    :return: Daughter radius
    """
    return ((radius ** exponent) / 2) ** (1 / exponent)


def RadiusToLength(radius, ratio=10):
    """
    Function to calculate the length of a vessel based on its radius.
