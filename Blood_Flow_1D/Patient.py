...
# -*- coding:utf-8 -*-
"""
Contains the Patient object class.
"""
import copy
import csv
import math
...
from Blood_Flow_1D import BloodFlowEquations, Constants, GeneralFunctions, Metadata, Perfusion, Results, Topology
...
class Patient:
    ...
    def __init__(self, folder=os.path.dirname(os.path.realpath(sys.argv[0]))):
        """
        Initiate the patient object with default values.
    ...
    def RedefineDirectionByFlowDirection(self):
        """
        Reverses the order of the nodes in each vessel if the flow inside the vessel is negative.
        This simple defines the positive flow direction to be the direction of flow.
        :return: Nothing
        """
        flowdict = {vessel[0]: vessel[1][0] for vessel in self.Results.MeanResults}

        for vessel in self.Topology.Vessels:
            # get the volumeflowrate
            volumeflowrate = flowdict[vessel.Name]
            if volumeflowrate < 0:
                vessel.Nodes.reverse()
                print("Vessel Reversed")

    def RemoveOldSimFiles(self):
        """
        Remove results from previous 1-D blood flow simulations.
        :return: Nothing
        """
    ...
    def ResetModellingFolder(self):
        ...
        blacklist = ["boundary_4&21&22&23&24&25&26&30.h5", "boundary_4&21&22&23&24&25&26&30.ply",
                     ...
                     "permeability_tensors.h5", "Model_parameters.txt", "PialSurface.vtp", "Distancemat.npy", "PA.vtp",
                     "DualPA.vtp"]
        ...
        # try:
        #     shutil.rmtree(self.Folders.PatientFolder + "logfile.log")
        # except:
        #     print("Error while deleting file ", folder + name)
    ...
    def LoadResults(self, file="Results.dyn", correct=True):
        ...
        for node, pressure, volumeflowrate, velocity, radius in zip(self.Topology.Nodes,
                                                                    self.Results.Pressure[0],
                                                                    self.Results.VolumeFlowRate[0],
                                                                    self.Results.Velocity[0],
                                                                    self.Results.Radius[0]):
            node.Pressure = pressure[0]
            node.FlowRate = volumeflowrate[0]
            node.Velocity = velocity[0]
            node.Radius = radius[0]
    ...
    def LoadPatientData(self, file="config.xml"):
        if file[-3:] == "txt":
            ...
            self.PatientData.LoadPatientDataYML(self.Folders.InputFolder + file)
        else:
    ...
    def LoadSegmentedVessels(self, CoWFile="1-D_Anatomy.txt"):
        self.Topology.LoadSegmentedVessels(self.Folders.InputFolder, CoWFile=CoWFile)
    ...
    def LoadPositions(self):
        vtpfile = self.Folders.ModellingFolder + "Topology.vtp"
        ...
        for index, node in enumerate(self.Topology.Nodes):
            node.SetPosition(list(pos[index]))
    ...
    def calculate_wk_parameters(self):
        ...
        for out in outletmap:
            part = len(Constants.fractions) - 1
            for index, body in enumerate(Constants.bodyparts):
                if out == body:
                    part = index
            bodymap.append(part)

        for bodynumber, fraction in enumerate(Constants.fractions):
    ...
    def calculate_wk_parameters_evenly(self):
        ...
        Calculate the windkessel parameters for the outlets of the network.
        """
        print("Calculating WindKessel Parameters, evenly through out the network.")
        scaling = 1e-3  # default for mm
    ...
    def SelectPatient(self):
        ...
        segmentationfolder = self.Folders.DataFilesFolder + "/Segmentations/"
    ...
    def SelectDonorNetwork(self):
        ...
        """
        scriptfolder = self.Folders.DataFilesFolder
    ...
    def UpdateModelParameters(self):
        ...
        self.ModelParameters["OUT_PRESSURE"] = self.PatientData["MeanRightAtrialPressure"]

        # self.PatientData["collaterals"]: collateral score, 0:absent, 1:poor, 2:moderate, 3:good
        collateral_map = {0: 0.05, 1: 0.25, 2: 0.5, 3: 0.65}  # detailed collateral model: probability
        self.ModelParameters["collateralpropability"] = collateral_map[self.PatientData["CollateralScore"]]

        collateral_map = {0: 0.0, 1: 0.05, 2: 0.14, 3: 0.3}  # two-way coupled model
        self.ModelParameters["coarse_collaterals_number"] = collateral_map[self.PatientData["CollateralScore"]]
    ...
    def WriteClotFile(self, name='Clots.txt'):
        print("Writing Clots to file.")
        with open(self.Folders.ModellingFolder + name, 'w') as f:
    ...
    def GenerateTrees(self, cutoff):
        print("Generating bifurcating arterial trees at the outlet nodes.")
        for id in self.Topology.OutletNodes:
            self.GenerateTree(id, cutoff)
        print("Generated %d bifurcating trees." % len(self.Trees))
    ...
    def LoadSurfaceMapping(self, filename="SurfaceNodesMapping.csv"):
        ...
        if len(self.Perfusion.CouplingPoints) == len(set([y for x in ClusterID for y in x])):
            for node, cluster in zip(NodeID, ClusterID):
                for id_number in cluster:
                    self.Perfusion.CouplingPoints[id_number].SurfaceNodes.append(node)
    ...
    def DistributeFlowVertex(self):
        ...
        # the only difference with the dual graph clustering is the type of data,
        # cell vs point and the cluster size might be different
        nodeids, _, _, clusterids, Areas, majorvesselid = self.LoadClusters()
        surfacenodes, clustermap, triangleArea = self.LoadSurfaceMapping()

        pointspercluster = [sum([1 for i in clustermap if i == id_number]) for id_number in clusterids]
    ...
    def GetMeanFlowRates(self):
        ...
        totalflow = 0
        regionsflowvolume = [0 for _ in range(0, 8)]
        clusterflowrate = []
        clusterPressure = []
        ...
        for index, node in enumerate(nodeids):
            ...
            pressuredatanode = [tp.Pressure[node] for tp in self.Results.TimePoints]
            pressureall = numpy.trapz(pressuredatanode, time)
            pressurefinal = pressureall / time[-1]
            clusterPressure.append(pressurefinal)
        ...
        return regionsflowrateregion, clusterflowrate, clusterPressure
    ...
    def DistributeFlowTriangles(self):
        ...
        time = [tp.WT for tp in self.Results.TimePoints]
        _, meanflowrate, _ = self.GetMeanFlowRates()
        flow = vtk.vtkFloatArray()
        flow.SetNumberOfComponents(1)
        flow.SetName("Volume Flow Rate (mL/s)")
        flowrate = vtk.vtkFloatArray()
    ...
    def ImportClotNodeFile(self):
        ...
        for id_number in uniqueclots:
            for index, clotid in enumerate(ClotID):
                if clotid == id_number:
                    clots[id_number].append((NodeID[index], Permeability[index], Porosity[index]))
    ...
    def ImportClots(self, file=""):
        ...
        for clot in Clots:
            ...
            try:
                ...
                # this will break compatibility with the pulsatile model so only do this when using the steady state model.
                if self.ModelParameters["Model"] == "Steady":
                    vessel.add_node_interp_length(location)
                    vessel.add_node_interp_length(location+length)
                ...
                print("Error: Likely caused by the clot file listing non-existent vessels.")
            self.Topology.UpdateTopology()
    ...
    def ExportTriangleFlowData(self):
        ...
        flowrateperregion, clusterflowrate, clusterpressure = self.GetMeanFlowRates()
        ...
        for triandleID, trianglemapping, area in zip(surfacenodes, clustermap, triangleAreas):
            flowPerTriangle[triandleID] = sum([flowPerAreaPerCluster[id_number] * area for id_number in trianglemapping])
        ...
        with open(self.Folders.ModellingFolder + "ClusterFlowData.csv", "w") as f:
            ...
            for index in range(0, numberofclusters):
                f.write("%d,%.10f,%d,%.10f\n" % (
                    index, clusterflowrate[index], trianglespercluster[index], clusterpressure[index]))

    def MajorArteriesToPialSurfaceNN(self, graph):
        """
        Map surface to to nearest node, colour based on nearest vessel.
        ...
        :return: graph.MajorVesselID is updated to the mapping.
        """
        ...
        for vessel in self.Topology.Vessels:
            # interpolation of nodes in the vessel
            nodeslengths = numpy.arange(0, vessel.Length, 100)
            x = vessel.InterpolationFunctions[0](nodeslengths)
            y = vessel.InterpolationFunctions[1](nodeslengths)
            z = vessel.InterpolationFunctions[2](nodeslengths)
            for xpos, ypos, zpos in zip(x, y, z):
                majornodes.append((xpos, ypos, zpos))
                majornodesID.append(vessel.MajorVesselID)
        ...
        meshcolour = [-1 for _ in graph.PialSurface]
    ...
    def WriteRegionMapping(self, filename="RegionMap.csv"):
        ...
        with open(file, 'w') as f:
            ...
            for index, node in enumerate(self.Topology.Nodes):
                ...
                f.write(str(node.Number) + "," + str(int(regionid)) + "\n")
    ...
    def LoadRegionMapping(self, filename="RegionMap.csv"):
        ...
        for nodedata in regiondata[1:]:
            ...
            self.Topology.Nodes[int(nodedata[0])].MajorVesselID = int(nodedata[1])
    ...
    def Initiate1DSteadyStateModel(self):
        ...
        outpressure = self.PatientData["MeanRightAtrialPressure"]
        ...
        for inlet in self.Topology.InletNodes:
            ...
            inlet[0].InletPressure = self.PatientData["SystolePressure"] * (1 / 3) + self.PatientData[
                "DiastolePressure"] * (2 / 3)
        ...
        for outlet in self.Topology.OutletNodes:
            ...
            outlet.OutletFlowRate = 0

    def Solve1DSteadyState(self, clotactive=False, PressureInlets=False, coarseCollaterals=False,
                           FlowRateOutlets=False, frictionconstant=22):
        visc = self.ModelParameters["BLOOD_VISC"]

        matrix, inputvector, nodes, edges, mapping = self.Topology.Get1DsteadyNetwork(visc, clotactive, PressureInlets,
                                                                                      coarseCollaterals,
                                                                                      FlowRateOutlets, frictionconstant)
        ...
        solution = solve(inputvector)
        # solution = spsolve(matrix, inputvector) # without pre-conditioning
        # print(f"Condition number: {numpy.linalg.cond(matrix)}")
        # print(info)
        # set pressure to all nodes
        # clotnodes = [node for clot in self.Topology.Clots for node in clot[0]]
        #
        # for node in clotnodes:  # internal clot nodes are not part of the solution from the solver.
        #     node.Pressure = 0
        ...
        # note that the collateral vessels alter the flow rates at the outlets.
        # the outlet flow rates therefore have to be summed over all connections
        # (should be zero for any bifurcation and internal vessel node)
        for edge in edges:
            edge[0].AccumulatedFlowRate = 0
            edge[1].AccumulatedFlowRate = 0
        ...
        for edge in edges:
            ...
            velocity = flowrate / (numpy.pi * radius * radius)
            edge[0].Velocity = velocity
            edge[1].Velocity = velocity
            # accumulated flow
            edge[0].AccumulatedFlowRate -= flowrate
            edge[1].AccumulatedFlowRate += flowrate

        if clotactive:
            for clot in self.Topology.Clots:
                clotnodes = clot[0]
                porocity = clot[2]
                for clotnode in clotnodes:
                    if porocity == 0:
                        ...
                        # effective area
                        area = numpy.power(clotnode.RefRadius, 2) * numpy.pi
                        clotnode.Velocity = clotnode.FlowRate / (area*porocity)
            ...
            #             edge[1].Velocity = 0
            #         else:
            #             edge[0].Velocity = edge[0].FlowRate/porocity
            #             edge[1].Velocity = edge[0].FlowRate/porocity

        # define flow rate at the outlet as the accumulation of flow (source term)
        for node in self.Topology.OutletNodes:
            if not (node.R1 is None):
                node.FlowRate = node.WKNode.AccumulatedFlowRate
    ...
    def UpdateOutletResistanceToPialSurface(self, PialSurfacePressure=9000):
        # todo update  to include C and R2 for pulsatile simulations
        for index, cp in enumerate(self.Perfusion.CouplingPoints):
            newresistancetotal = (cp.Node.Pressure - PialSurfacePressure[index]) / (cp.Node.FlowRate * 1e-6)
            cp.Node.R1 = newresistancetotal  # - cp.Node.R1
            cp.Node.R2 = 0
            print("Resistance: %f" % newresistancetotal)
            if newresistancetotal < 0:
                # for index, cp in enumerate(self.Perfusion.CouplingPoints):
                #     print("1D model: %f - Perfusion model: %f\n" % (cp.Node.Pressure, PialSurfacePressure[index]))
                raise ValueError(
                    "Error: Negative resistance. Pial surface pressure is higher than the arterial pressure.")
            # if cp.Node.R2 < 0:
            #     cp.Node.R2 = 0
            # cp.Node.R1 = newresistancetotal - cp.Node.R2
            # print("Error: R2<0. Setting R2 to 0.1e9 and updating R1.")
            cp.Node.OutPressure = PialSurfacePressure[index]  # pressure now fixed to this value at this node

    def UpdatePressureCouplingPoints(self, pressures):
        """
        Update the pressure at the coupling points to the perfusion model.
        ...
        try:
            ...
            for index, cp in enumerate(self.Perfusion.CouplingPoints):
                # cp.Node.R1 = None  # remove wk elements
                cp.Node.OutPressure = pressures  # pressure now fixed to this value at this node

    def UpdateFlowRateCouplingPoints(self, flowrates):
        """
        Update the flowrate at the coupling points to the perfusion model.
        ...
        try:
            for index, cp in enumerate(self.Perfusion.CouplingPoints):
                # cp.Node.R1 = None  # remove wk elements
                cp.Node.OutletFlowRate = flowrates[index]  # flowrate now fixed to this value at this node
        except TypeError:
            for index, cp in enumerate(self.Perfusion.CouplingPoints):
                # cp.Node.R1 = None  # remove wk elements
                cp.Node.OutletFlowRate = flowrates  # flowrate now fixed to this value at this node

    def Run1DSteadyStateModel(self, model="Linear", tol=1e-10, clotactive=False, PressureInlets=False,
                              coarseCollaterals=False, FlowRateOutlets=False, frictionconstant=22,
                              scale_resistance=True):
        if clotactive:
            print("Solving steady state: stroke scenario.")
        else:
            print("Solving steady state: healthy scenario.")
        # initialise all values
        start = time.time()

        solutionold = self.Solve1DSteadyState(clotactive, PressureInlets, coarseCollaterals,
                                              FlowRateOutlets, frictionconstant)
        if model == "NonLinear":
        ...
        while True:
            if model == "Linear":
                solutionnew = self.Solve1DSteadyState(clotactive, PressureInlets, coarseCollaterals,
                                                      FlowRateOutlets, frictionconstant)
            ...
            # scale resistances to get both proper flow and pressure at the inlet
            if scale_resistance:
                pressure_diff = self.Topology.InletNodes[0][0].Pressure/self.Topology.InletNodes[0][0].InletPressure
                flow_rate_diff = 1e-6 * self.Topology.InletNodes[0][0].FlowRate/self.Topology.InletNodes[0][0].InletFlowRate
                if abs(pressure_diff-1) > 1e-6 or abs(flow_rate_diff-1) > 1e-6:
                    print("Detected a difference in inlet boundary conditions, scaling outlet resistance.")
                    # print(self.Topology.InletNodes[0][0].Pressure)
                    # print(self.Topology.InletNodes[0][0].InletPressure)
                    # print(1e-6 * self.Topology.InletNodes[0][0].FlowRate)
                    # print(self.Topology.InletNodes[0][0].InletFlowRate)

                    print(f"\tPressure ratio:{pressure_diff}")
                    print(f"\tFlow Rate ratio:{flow_rate_diff}")
                    for node in self.Topology.OutletNodes:
                        # print(node.R2)
                        # total resistance of the outlet needs to change, but R1 is constant
                        # scale R2 to compensate, (r1+r2)*p = r1+r2*F
                        # assumption is also that either pressure or flow rate diff is equal to one already
                        fraction = ((node.R1+node.R2)/(pressure_diff*1/flow_rate_diff)-node.R1)/node.R2
                        node.R2 = fraction*node.R2
                    continue
            difference = numpy.linalg.norm(
                [solutionold[i] - solution for i, solution in enumerate(solutionnew)]) / numpy.linalg.norm(solutionnew)
            print("\tConvergence criteria %0.12g" % difference)
            solutionold = solutionnew
            iter += 1

            if difference < tol:
                print("\tSteady state convergence in %d iterations." % iter)
                time_elapsed = time.time() - start
                print('\tTime elapsed %f seconds' % time_elapsed)
                break
        ...
        for bif in self.Topology.BifurcationNodes:
            ...
            bif.FlowRate = 0  # numpy.mean([node.FlowRate for node in list(bif.Connections)])
            bif.Velocity = 0  # numpy.mean([node.Velocity for node in list(bif.Connections)])

        if coarseCollaterals:
            totalflowrate = 0
            for edge in self.Topology.Collaterals:
                dp = edge[0].Pressure - edge[1].Pressure
                totalflowrate += abs(dp * edge[2] * 1e6)
            print(f"\tTotal Collateral flow: {totalflowrate} mL/s")
        # for node in self.Topology.OutletNodes:
        #     print(node.WKNode.AccumulatedFlowRate)
        # totaloutflow = sum([node.WKNode.AccumulatedFlowRate for node in self.Topology.OutletNodes])
        # print(f"Total outflow: {totaloutflow}")
    ...
    def Results1DSteadyStateModel(self):
        ...
        vesselnodes = [vessel.Nodes for vessel in self.Topology.Vessels]
        for index, vessel in enumerate(vesselnodes):
            meanvalues = self.Results.CalculateMeanVessel(vessel)
    ...
    def ExportClotBFValues(self, file="BFClotValues.csv"):
        with open(self.Folders.ModellingFolder + file, "w") as f:
            f.write("Clot index,Flow rate(mL/s),Pressure drop(Pa),Proximal pressure (Pa),Distal pressure (Pa),Pressure ratio\n")
            for index, clot in enumerate(self.Topology.Clots):
                node1 = min(clot[0], key=lambda item: item.Number)
                node2 = max(clot[0], key=lambda item: item.Number)
                proximal_pressure = node1.Pressure
                distal_pressure = node2.Pressure
                pressure_drop = proximal_pressure - distal_pressure
                pressure_ratio = distal_pressure/proximal_pressure
                flow_rate = node1.FlowRate
                f.write("%d,%.10f,%.10f,%.10f,%.10f,%.10f\n" % (index, flow_rate, pressure_drop, proximal_pressure,
                                                                distal_pressure, pressure_ratio))
