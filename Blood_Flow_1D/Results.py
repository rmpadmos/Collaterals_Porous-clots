...
# -*- coding:utf-8 -*-
"""
Contains the Results object class.
"""
import time
...
class Results:
    ...
    def CalculateMeanVessel(self, ids):
        ...
        # todo note that velocity of clot nodes is not calculated properly here
    ...
    def CalculateMeanResultsPerNode(self):
        for index, heartbeat in enumerate(self.Pressure):
            pulseindex = [(numpy.max(measurementpernode) - numpy.min(measurementpernode)) /
                          ...
                          if integrate.simps(measurementpernode, self.Time[index]) > 0 else 0
            ...
            velocity = [volumeflowrate[i] / (numpy.pi * radius[i] * radius[i]) if radius[i] > 0 else 0 for i in range(0, len(radius))]
            ...
            # pulseindexvelocity = [(numpy.max(measurementpernode) - measurementpernode[timeindexDiastolicPressure[i]]) / abs(meanvelocity[i]) for i, measurementpernode in enumerate(self.Velocity[index])]
            pulseindexvelocity = [max(0, abs((numpy.max(measurementpernode) - numpy.min(measurementpernode)) /
                                             meanvelocity[i])) if meanvelocity[i] > 0 else 0 for i, measurementpernode in
                                  enumerate(self.Velocity[index])]
    ...
    def plotallprofiles(self, ids, name="", figuredpi=72, legend=""):
        self.fig = plt.figure()
        ...
        if legend:
            self.fig.subplots_adjust(bottom=0.3, wspace=0.2)
            self.ax2.legend(labels=[legend[0], legend[1], legend[2]], loc='upper center', bbox_to_anchor=(0.5, -0.2),
                            fancybox=False, shadow=False, ncol=3, fontsize=30)

        if name:
            self.fig.savefig(name, dpi=figuredpi, bbox_inches='tight')
        plt.show()
